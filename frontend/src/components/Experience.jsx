...
function ExperienceCard({ exp }) {
  return (
    <motion.div
      ...
      className="glass rounded-xl p-6 hover:shadow-[0_0_28px_rgba(124,58,237,0.2)] transition-shadow duration-300"
    >
      <span className="inline-block px-3 py-1 bg-violet-500/15 text-violet-400 rounded-full text-xs font-semibold mb-3 border border-violet-500/20">
        {exp.startDate} – {exp.endDate}
      </span>
      <h3 className="text-xl font-bold text-white mb-0.5">{exp.role}</h3>
      <p className="text-violet-400 font-medium text-sm mb-1">{exp.company}</p>
      {exp.location && (
      ...
      <ul className="space-y-1.5 mb-4">
        {exp.bullets.map((bullet, i) => (
          <li key={i} className="flex items-start gap-2 text-gray-400 text-sm leading-relaxed">
            <span className="w-1.5 h-1.5 rounded-full bg-violet-500/70 flex-shrink-0 mt-[7px]" />
            {bullet}
          </li>
        ))}
      </ul>
  ...
  return (
    <section id="experience" className="py-28 bg-dark-800 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/4 w-[28rem] h-[28rem] bg-violet-500/8 rounded-full blur-3xl" />
        <div className="absolute bottom-0 right-1/4 w-[28rem] h-[28rem] bg-blue-500/8 rounded-full blur-3xl" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="relative mb-28">
          {/* Vertical line */}
          <div className="absolute left-6 md:left-1/2 top-0 h-full w-px bg-gradient-to-b from-violet-500 via-blue-500/70 to-violet-500/10 md:-translate-x-px" />
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-6 md:left-1/2 top-8 -translate-x-1/2 z-10">
                    <div className="w-3.5 h-3.5 rounded-full bg-violet-500 ring-4 ring-dark-800" />
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {education.map((edu, index) => (
            <motion.div
              ...
              className="glass rounded-xl p-6 hover:shadow-[0_0_28px_rgba(124,58,237,0.18)] transition-shadow duration-300"
            >
              <span className="inline-block px-3 py-1 bg-blue-500/15 text-blue-400 rounded-full text-xs font-semibold mb-3 border border-blue-500/20">
                {edu.startDate} – {edu.endDate}
              </span>
              <h3 className="text-base font-bold text-white mb-1 leading-snug">{edu.degree}</h3>
              <p className="text-violet-400 font-medium text-sm mb-3">{edu.school}</p>
